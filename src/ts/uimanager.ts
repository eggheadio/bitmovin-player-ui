  ...
  constructor(
    ...
    for (let uiVariant of this.uiVariants) {
      ...
      this.uiInstanceManagers.push(
        new InternalUIInstanceManager(player, uiVariant.ui, this.config)
      )
    }
    ...
    let resolveUiVariant = (event: PlayerEvent) => {
      ...
      for (let uiVariant of this.uiVariants) {
        if (
          ...
          uiVariant.condition(context) === true
        ) {
      ...
      if (uiVariantChanged) {
        ...
        if (this.currentUi != null) {
          ...
          if (context.isAd) {
            ...
            this.currentUi
              .getWrappedPlayer()
              .fireEventInUI(this.player.EVENT.ON_AD_STARTED, adStartedEvent)
          }
    ...
    this.managerPlayerWrapper
      .getPlayer()
      .addEventHandler(this.player.EVENT.ON_AD_STARTED, resolveUiVariant)
    this.managerPlayerWrapper
      .getPlayer()
      .addEventHandler(this.player.EVENT.ON_AD_FINISHED, resolveUiVariant)
    this.managerPlayerWrapper
      .getPlayer()
      .addEventHandler(this.player.EVENT.ON_AD_SKIPPED, resolveUiVariant)
    this.managerPlayerWrapper
      .getPlayer()
      .addEventHandler(this.player.EVENT.ON_AD_ERROR, resolveUiVariant)
    this.managerPlayerWrapper
      .getPlayer()
      .addEventHandler(this.player.EVENT.ON_PLAYER_RESIZE, resolveUiVariant)
    this.managerPlayerWrapper
      .getPlayer()
      .addEventHandler(this.player.EVENT.ON_FULLSCREEN_ENTER, resolveUiVariant)
    this.managerPlayerWrapper
      .getPlayer()
      .addEventHandler(this.player.EVENT.ON_FULLSCREEN_EXIT, resolveUiVariant)
...
export namespace UIManager.Factory {
  export function buildEggheadUI(
    player: Player,
    config: UIConfig = {}
  ): UIManager {
    ...
    return new UIManager(player, eggheadUI, config)
  }
  ...
  function legacyCastReceiverUI() {
    let controlBar = new ControlBar({
      components: [new SeekBar(), new PlaybackTimeLabel()]
    })
  ...
  export function buildLegacyCastReceiverUI(
    player: Player,
    config: UIConfig = {}
  ): UIManager {
  ...
  export function buildLegacyTestUI(
    player: Player,
    config: UIConfig = {}
  ): UIManager {
    ...
    UIUtils.traverseTree(component, component => {
      ...
      for (let configuredComponent of configuredComponents) {
        if (configuredComponent === component) {
          ...
          throw Error(
            "Circular reference in UI tree: " + component.constructor.name
          )
        }
      }
